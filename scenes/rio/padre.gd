class_name Padre_rio extends CharacterBody2D

@export var velocidad_caminar := 100
@export var velocidad_correr := 150

var direccion := Vector2.ZERO
var corriendo := false

@export var anim: AnimatedSprite2D;
@export var area_collect: Area2D;
...
func _ready():
	add_to_group("player");
	area_collect.connect("body_entered", on_body_entered);
	area_collect.connect("body_exited", on_body_exited);
...
func _physics_process(_delta: float) -> void:
	...
	if Input.is_action_just_pressed("interact"):
		if objeto_para_recoger != null:
			...
			var dir_anim = _obtener_direccion_idle()
